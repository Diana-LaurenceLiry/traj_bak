...
def process_one_bag(bag_path, args, stamp):
    raw_bag_name = os.path.splitext(os.path.basename(bag_path))[0]
    bag_name = args.name_override.strip() if args.name_override else raw_bag_name
    method = infer_method(raw_bag_name, args.method)
    ...
    if HAS_MPL:
        ...
        # Plot: relative-position figure per dynamic obstacle.
        if dyn_rel_samples:
            by_name = {}
            ...
            for rel_name in sorted(by_name.keys()):
                ...
                rx_s, ry_s, rd_s = smooth_rel_series(rx, ry, rd, med_win=9, mean_win=7)
...
def main():
    ...
    parser.add_argument("--name_override", default="", help="Override output name/title prefix for this bag")
